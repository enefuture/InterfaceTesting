...
class Handlers
{
    ...
    */
    ...
    //自定义参数设置
    public static var filterUrl = ['api.test2.com','api.test2.com'];//TODO 替换此处接口域名 
    public static var filePath = "D:\\FiddlerFilterApi" ; //收集接口数据存放目录
    public static var projectName = "Pop" + "_" + getNowFormatDate(2);// 生成当天测试项目目录
    //文件流的声明使用
    static var fso = new ActiveXObject("Scripting.FileSystemObject");
    static var file;
    static var isCreateFolder = false;
...
*/
    ...
    //添加的划分接口文件的方法,根据url的action方法
    static function DivideUrl(url){
        var apiFolder = filePath+ "\\" + projectName + "\\" + url.Split("/")[0] + "\\";
        var filename = "";
        url = url.replace(/\//g,"_");
        var urlSet = url.Split("?");
        filename = urlSet[0];
        return [apiFolder,filename];
    }
    //创建接口文件目录，与上述个人配置的filterUrl数组名相匹配
    static function createFolder(){
        if(!fso.FolderExists(filePath)){
            fso.CreateFolder(filePath);
        }
        var root_path = filePath + "\\" + projectName;
        if(!fso.FolderExists(root_path)){
            fso.CreateFolder(root_path);
        }
        for(var url in filterUrl){
            var apiFolder = root_path + "\\" + filterUrl[url];
            if(fso.FolderExists(apiFolder)){
                continue;
            }
            else{
                fso.CreateFolder(apiFolder);
            }
        }
    }
    static function getNowFormatDate(format){
        var date = new Date();
        var seperator1 = "-";
        var seperator2 = ":";
    ...
    static function OnBeforeResponse(oSession: Session) {
        ...
        for(var url in filterUrl){
            //过滤无关请求，只关注特定请求
            if (oSession.fullUrl.Contains(filterUrl[url]))
            {
                var responseBody = oSession.GetResponseBodyAsString();
                var url = oSession.url;
                var writeFile_array  =  DivideUrl(url);
                var write_folder = writeFile_array[0];
                var filtername = writeFile_array[1];
                if(fso.FolderExists(write_folder)){
                    // var testfile = filePath+ "\\" + "test.txt"
                    var myDate = getNowFormatDate(1);
                     // 首先存储Response code不等于200的情况，该请求很大可能服务器发生错误。
                    if (oSession.responseCode != 200){
                        oSession.utilDecodeResponse();//消除保存的请求可能存在乱码的情况
                        var writeFile = write_folder + "\\" + filtername + "_errorApi.txt";
                        file = fso.OpenTextFile(writeFile,8 ,true, true);
                        file.writeLine("\n");
                        file.writeLine("Request date: " + myDate);
                        var http = oSession.isHTTPS ? "https://" : "http://";
                        file.writeLine("Request http: " + http);
                        file.writeLine("Request url: " + url);
                        file.writeLine("Request header: " + oSession.oRequest.headers);
                        file.writeLine("Request body: " + oSession.GetRequestBodyAsString());
                        file.writeLine("Response code: " + oSession.responseCode + "\r\n");
                        responseBody = unescape(responseBody.replace(/\\u/g,"%u"));
                        ...
                        oSession.utilDecodeResponse();//消除保存的请求可能存在乱码的情况
                        var writeFile = write_folder + "\\" + filtername + "_SuccessApi.txt";
                        //保存服务器返回StatsCode为200的请求
                        file = fso.OpenTextFile(writeFile,8 ,true, true);
                        file.writeLine("\n");
                        // file.writeLine("wirteFile: " + writeFile);
                        file.writeLine("Request date: " + myDate);
                        var http = oSession.isHTTPS ? "https://" : "http://";
                        file.writeLine("Request http: " + http);
                        file.writeLine("Request url: " + url);
